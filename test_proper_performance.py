#!/usr/bin/env python3
"""
Proper performance test with simple baseline
"""
import sys
import os
...
import time
import statistics
from simple_baseline import SimpleVAEDecode
...
def test_performance():
    ...
    print("=== Proper Performance Comparison Test ===")
    ...
    print("\nTesting Simple Baseline VAE Decode...")
    baseline_node = SimpleVAEDecode()
    baseline_times = []
    
    for i in range(10):
        start_time = time.time()
        try:
            result = baseline_node.decode(
                vae=vae,
                samples=samples,
                tile_size=512,
                overlap=64,
    ...
    if baseline_times and v2_fixed_times:
        baseline_avg = statistics.mean(baseline_times)
        v2_fixed_avg = statistics.mean(v2_fixed_times)
        improvement = ((baseline_avg - v2_fixed_avg) / baseline_avg) * 100
        ...
        if improvement < 0:
            print(f"⚠️  Performance regression: {abs(improvement):.1f}% slower")
        else:
