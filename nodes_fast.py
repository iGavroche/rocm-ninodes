#!/usr/bin/env python3
"""
ACTUALLY OPTIMIZED VAE Decode - Simple and Fast
Based on analysis: the simple baseline is fastest, so let's optimize that
"""
import torch
...
class ROCMOptimizedVAEDecodeFast:
    """
    ACTUALLY Optimized VAE Decode - Simple and Fast
    Based on analysis: simple baseline is fastest, so optimize that approach
    """
    ...
    def decode(self, vae, samples, tile_size=768, overlap=96, use_rocm_optimizations=True, 
        ...
        try:
            ...
            # Determine optimal precision - SIMPLE
            ...
            if len(samples_processed.shape) == 4:  # Image
                ...
                # Try direct decode first - OPTIMIZED for gfx1151
                try:
                    # Use optimal autocast for gfx1151
    ...
    def _decode_tiled_optimized(self, vae, samples, tile_size, overlap):
        """Optimized tiled decode - minimal overhead, maximum speed"""
        samples_processed = samples["samples"]
        ...
        # Pre-allocate output tensor - OPTIMIZED allocation
        ...
        # Process tiles with MINIMAL overhead - OPTIMIZED for gfx1151
        for i in range(tiles_h):
            for j in range(tiles_w):
                ...
                # Extract tile - NO unnecessary copying
                tile_samples = samples_processed[:, :, start_h:end_h, start_w:end_w]
                
                # Decode tile - OPTIMIZED for gfx1151
...
@instrument_node
class ROCMOptimizedVAEDecodeFastInstrumented(ROCMOptimizedVAEDecodeFast):
    pass

# Node class mapping for ComfyUI
NODE_CLASS_MAPPINGS = {
    "ROCMOptimizedVAEDecodeFast": ROCMOptimizedVAEDecodeFastInstrumented
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "ROCMOptimizedVAEDecodeFast": "ROCM Optimized VAE Decode Fast"
}
