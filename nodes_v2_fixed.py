#!/usr/bin/env python3
"""
FIXED Phase 1 Optimization Implementation for ROCMOptimizedVAEDecode
Focus: ACTUAL Memory Management and Tile Size Optimization
"""
import torch
...
class ROCMOptimizedVAEDecodeV2Fixed:
    """
    FIXED Phase 1 Optimized VAE Decode node for gfx1151 architecture.
    
    ACTUAL Optimizations:
    - Simplified memory management (no overhead)
    - Smart tile size selection
    - Reduced tensor copying
    - Minimal overhead optimizations
    """
    
    def __init__(self):
        # Simple tile size cache (no complex memory pooling)
        self.tile_cache = {}
    ...
    def _get_optimal_tile_size(self, width: int, height: int, vae) -> Tuple[int, int]:
        """Get optimal tile size based on input dimensions - SIMPLIFIED"""
        ...
        # Simple cache (no complex logic)
        self.tile_cache[cache_key] = (optimal_tile, optimal_tile)
        return (optimal_tile, optimal_tile)
    
    def _decode_tiled_simple(self, vae, samples, tile_size, overlap):
        """SIMPLIFIED tiled decode - minimal overhead"""
        ...
        # Pre-allocate output tensor - SIMPLE allocation
        ...
        # Process tiles with MINIMAL overhead
    ...
    def decode(self, vae, samples, tile_size=768, overlap=96, use_rocm_optimizations=True, 
        ...
        """
        FIXED Phase 1 Optimized VAE decode for ROCm/AMD GPUs
        """
        try:
            samples_processed = samples["samples"]
            
            # Determine optimal precision - SIMPLIFIED
            ...
            if len(samples_processed.shape) == 4:  # Image
                ...
                # Try direct decode first - SIMPLIFIED
                try:
            ...
            # Fallback to SIMPLIFIED tiled decode
            try:
                ...
                pixel_samples = self._decode_tiled_simple(vae, samples, tile_size, overlap)
...
@instrument_node
class ROCMOptimizedVAEDecodeV2FixedInstrumented(ROCMOptimizedVAEDecodeV2Fixed):
    pass

# Node class mapping for ComfyUI
NODE_CLASS_MAPPINGS = {
    "ROCMOptimizedVAEDecodeV2Fixed": ROCMOptimizedVAEDecodeV2FixedInstrumented
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "ROCMOptimizedVAEDecodeV2Fixed": "ROCM Optimized VAE Decode V2 Fixed"
}
