#!/usr/bin/env python3
"""
Quick performance test to identify the slowdown issue
"""
import sys
import os
...
def test_performance():
    ...
    # Test V2 node
    print("\nTesting ROCMOptimizedVAEDecodeV2...")
    v2_node = ROCMOptimizedVAEDecodeV2Instrumented()
    v2_times = []
    
    for i in range(10):
        start_time = time.time()
        try:
            result = v2_node.decode(
                vae=vae,
                samples=samples,
                tile_size=512,
                overlap=64,
            ...
            execution_time = time.time() - start_time
            v2_times.append(execution_time)
            print(f"  Iteration {i+1}: {execution_time:.6f}s")
        except Exception as e:
            print(f"  Iteration {i+1}: FAILED - {e}")
    
    # Calculate statistics
    if original_times and v2_times:
        original_avg = statistics.mean(original_times)
        v2_avg = statistics.mean(v2_times)
        improvement = ((original_avg - v2_avg) / original_avg) * 100
        
        print(f"\n=== Results ===")
        print(f"Original Average: {original_avg:.6f}s")
        print(f"V2 Average: {v2_avg:.6f}s")
